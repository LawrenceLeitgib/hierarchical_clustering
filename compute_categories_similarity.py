...
def extract_abstracts_and_categories(num_samples,mc,balance):
    with open('resources/arxiv-metadata-oai-snapshot.json') as f:
        ...
        for line in f:
            ...
            if(len(categories_dict.keys())==mc and c not in categories_dict):
                continue

            if(categories_dict.get(c,0)>=((1+balance)*(num_samples/mc)) and balance!=-1):
                continue
    ...
    return abstract,categories_dict,categories_list
